...
class SettingsPage(QWidget):
    ...
    def _build_transparent_card(self) -> QFrame:
        frame, layout = _card("Modo transparente")
        settings = self.ctx.config.settings
        ...
        form = QFormLayout()
        self.transparent_port_spin = QSpinBox()
        self.transparent_port_spin.setRange(1024, 65535)
        self.transparent_port_spin.setValue(settings.transparent_port)
        form.addRow("Porta do listener transparente", self.transparent_port_spin)
        layout.addLayout(form)
        ...
        save_btn.setObjectName("PrimaryButton")
        save_btn.clicked.connect(self._on_save_transparent)
        row = QHBoxLayout()
        row.addStretch(1)
        row.addWidget(save_btn)
        layout.addLayout(row)

        self.transparent_result_label = QLabel("")
        self.transparent_result_label.setWordWrap(True)
        layout.addWidget(self.transparent_result_label)
        return frame

    def _on_save_transparent(self) -> None:
        settings = self.ctx.config.settings
